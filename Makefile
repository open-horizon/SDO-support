...
#FDO_IMAGE_LABELS ?= --label 'vendor=IBM name=$(FDO_DOCKER_IMAGE) version=$(VERSION) release=$(shell git rev-parse --short HEAD) summary="Open Horizon SDO support image" description="The SDO owner services run in the context of the open-horizon management hub"'
...
.PHONY: default run-$(FDO_DOCKER_IMAGE) push-$(FDO_DOCKER_IMAGE) publish-$(FDO_DOCKER_IMAGE) promote-$(FDO_DOCKER_IMAGE) pull-$(FDO_DOCKER_IMAGE) clean
