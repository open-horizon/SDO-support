...
docker run --name $SDO_MARIADB_DOCKER_IMAGE --network $DOCKER_NETWORK -dt -p $SDO_MARIADB_PORT:$SDO_MARIADB_PORT -e "MYSQL_DATABASE=$MYSQL_DATABASE" -e "MYSQL_ROOT_PASSWORD=$MYSQL_ROOT_PASSWORD" -e "MYSQL_USER=$MYSQL_USER" -e "MYSQL_PASSWORD=$MYSQL_PASSWORD" -e "TZ=$TZ" $DOCKER_REGISTRY/$SDO_MARIADB_DOCKER_IMAGE:$VERSION
